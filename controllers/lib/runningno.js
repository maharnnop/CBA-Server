...
const getRunNo = async (runtype,paramclass,subclass,usercode,effdate,t) => {
    ...
    try {
        ...
        //table TURUNT
          const basisResult = await sequelize.query(`SELECT "PeriodBasis", "fixdigit" FROM static_data."TURUNT" WHERE "RunType" = '${RunType}'`, { transaction: t,type: sequelize.QueryTypes.SELECT });
    if (basisResult.length === 0) {
      throw new Error(`RunType: ${RunType} not found in TURUNT !`);
    }
    const basis = basisResult[0].PeriodBasis.trim();
    fixdigit = basisResult[0].fixdigit
    ...
    //   };
      ...
      if (basis !== '') {
        if (basis === 'D') {
          ...
          condition = condition +  ` and EXTRACT(YEAR FROM "EffectiveDate") = '${EffectiveDate.split('-')[0]}'` 
          if (basis === 'M') {
            condition = condition + ` and EXTRACT(YEAR FROM "EffectiveDate") = '${EffectiveDate.split('-')[0]}'  AND EXTRACT(MONTH FROM "EffectiveDate") = '${EffectiveDate.split('-')[1]}'  `
          }
        }
      }
    ...
    if (updatedTurunResult[0]) {
      ...
      await sequelize.query(`insert into static_data."TURUN" ("RunID", "RunType", "Class", "SubClass", "EffectiveDate", "LastNo",  "UpdateUserCode", "fixdigit") 
      values (${RunID}, '${RunType}', ${pClass}, ${pSubClass}, '${EffectiveDate}', ${RunNo}, '${UpdateUserCode}',  ${fixdigit})`, { transaction: t, type: sequelize.QueryTypes.update,});
    }
    // await t.commit();
} catch (error) {
  console.error(error)
...
if (fixdigit !== null) {
// console.log('RunNo : ' + String(RunNo).padStart(fixdigit, '0'));
  return String(RunNo).padStart(fixdigit, '0')
