...
const recursivediff = (obj , data, oldval , newval) =>{
  console.log(`>>> typeof : ${typeof(obj)}`);
  console.log(`>>> isArray : ${Array.isArray(obj)}`);
  if (Array.isArray(obj)){
    for (let i = 0; i < obj.length; i++) {
      recursivediff(obj[i], data , oldval[i],newval[i])
      
    }
  }
  else if (typeof(obj) == 'object') {
    Object.keys(oldval).forEach(key => {
       console.log(`>>> typeof oldval [${key}]: ${typeof(oldval[key])}`);
       if (Array.isArray(oldval[key])){
          recursivediff(obj[key], data , oldval[key],newval[key])
       }else if (typeof(oldval[key]) == 'object' ) {
          recursivediff(obj[key],data,oldval[key],newval[key])
...
and t.status = 'N' order by bj."seqNo" ;`,
    ...
    const requestNo = `REQ-${getCurrentYY()}` + await getRunNo('reqiedt', null, null, 'kw', currentdate, t);
...
VALUES(null, :previousid, :requestNo, :category, 'N',  :usercode, null, null, now() ) returning id;`,
      {
        replacements: {
          // polid: policy.polid,
    ...
    //#region insert b_juiedtds
    // policy
    let value = { "seqNoagt": policy.seqNoagt, specdiscamt: policy.specdiscamt, endorseNo: null }
...
, :tableM1, :keyidM1, null, null, null, null);`,
    ...
    await t.commit()
    await res.json({ requestNo: requestNo })
  } catch (error) {
    await t.rollback();
...
const endorseSpecdisc = async (t, policy, usercode) => {
  console.log('------ start endorseSpecdisc ---------');

  const currentdate = getCurrentDate()
  try {

    policy.edtypecode === "MT81"

    const juiedtds = await sequelize.query(
      `select bj2.* from static_data.b_juiedts bj 
join static_data.b_juiedtds bj2 on bj2.keyidm =bj.id
where bj.request_no = :request_no ;`,
      {
        replacements: {
          request_no: policy.request_no
        },
        transaction: t,
        type: QueryTypes.SELECT
      })
    const update_pol = juiedtds.find(({ table }) => table === `static_data."Policies"`);
    const update_jupgr = juiedtds.find(({ table }) => table === `static_data.b_jupgrs`);

    for (const key in update_pol.value) {
      policy[key] = update_pol.value[key] // update seqNoagt, endorseNo, specdiscamt
    }
    policy.installment = update_jupgr.value
    await sequelize.query(
      `update static_data."Policies" 
             SET "policystatus" = 'ED', "lastVersion" = 'N'
            WHERE  id = :polid 
            -- "lastVersion" = 'Y' and "policyNo" = :policyNo `,
      {
        replacements: {
    ...
    // let cond = `"endorseNo" = :endorseNo`
    // if (req.body.endorseNo === null) {
    //   cond = `"endorseNo" is null`
    // }
   ...
   and dfrpreferno is null `,
    ...
    policy.createusercode = usercode
    policy.endorseNo = endorseNo
    policy.applicationNo = applicationNo
    policy.id = null

    console.log(endorseNo);
    policy.insurancestatus = 'AA'
    policy.policystatus = 'PC'
    ...
    await sequelize.query(
      `insert into static_data."b_juiepcs" 
             ("polid", previousid, request_no, "endorseNo", edeffdate, edexpdate) values
            (:polid, :previousid, :request_no, :endorseNo, :edeffdate, :edexpdate)`,
      {
        replacements: {
          polid: policy.polid,
          previousid: policy.previousid,
          ...
          edeffdate: policy.actDate,
          edexpdate: policy.expDate,
        },
        transaction: t,
        type: QueryTypes.INSERT
      }
    ...
    await sequelize.query(
        ...
        WHERE request_no = :request_no`,
      {
        replacements: {
          polid: policy.polid,
          update_by: usercode,
          detail: `DISC-IN ${policy.specdiscamt}`,
          request_no: policy.request_no
        },
        transaction: t,
        type: QueryTypes.INSERT
    ...
    await sequelize.query(
      `insert into static_data."b_juiepms" 
      ("polid", diffnetgrossprem, request_no, "diffduty", difftax, difftotalprem, discinamt) values
      (:polid, :diffnetgrossprem, :request_no, :diffduty, :difftax, :difftotalprem, :discinamt)`,
      {
        replacements: {
          polid: policy.polid,
          request_no: policy.request_no,
    ...
    console.log(">>> done insert juepc juedt juepm ");
    const jupgr = {}
    // const installmentEdit = req.body.installment.advisor.filter(ele => !ele.editflag)
    jupgr.advisor = policy.installment.advisor
    await createjupgrChangeinv(policy, t, usercode)
...
const endorseChangeinv = async (req, res) => {
  ...
  try {
    ...
    const oldPolicy = await sequelize.query(
      `select "seqNoagt", specdiscamt   from static_data."Policies" 
     WHERE id = :polid ;`,
      {
        replacements: {
          polid: req.body.previousid
...
and t.status = 'N' order by bj."seqNo" ;`,
   ...
   `,
    ...
    console.log(">>> insert b_juiepcs done");

    const b_juiedts = await sequelize.query(
      `insert into static_data."b_juiedts" 
      ("polid", "previousid", request_no, status, request_from, update_by, note, request_date, category) values
      (:polid, :previousid, :request_no, 'A', :request_from, 'Autoapprove', :note, now(), 'ChangeInvoice') returning id;`,
      {
        replacements: {
          polid: req.body.polid,
          previousid: req.body.previousid,
          request_no: requestNo,
          request_from: usercode,
          note: "MT82 แก้ไขงวด",
    ...
    console.log(">>> insert b_juiedts done id : " + b_juiedts[0][0].id);



    console.log(`>>> previousid : ${req.body.previousid}`);

    const keyidm = b_juiedts[0][0].id
    //#region insert b_juiedtds
    // policy
    let value = { "seqNoagt": req.body.seqNoagt, "specdiscamt": req.body.specdiscamt }
    let old_value = oldPolicy[0]
    await sequelize.query(
      `INSERT INTO static_data.b_juiedtds
(keyidm, category, field, value, old_value, "table", keyid
...
, null, null, null, null, null, null);`,
      {
        replacements: {
          keyidm: keyidm,
          category: 'ChangeInvoice',
          value: JSON.stringify(value),
          old_value: JSON.stringify(old_value),
          table: 'static_data."Policies"',
          keyid: req.body.previousid
        },
        transaction: t,
        type: QueryTypes.INSERT
      }
...
// MT83
const requestEdtCommOV = async (req, res) => {
  const jwt = req.headers.authorization.split(' ')[1];
  const usercode = decode(jwt).USERNAME;
  const appNo = []
  ...
  try {
    ...
    const policy = req.body
    const oldPolicy = await sequelize.query(
      `SELECT  commin_rate, commin_amt, ovin_rate, ovin_amt, commin_taxamt, ovin_taxamt,
       commout1_rate, commout1_amt, ovout1_rate, ovout1_amt, 
       commout2_amt, ovout2_rate, ovout2_amt, commout2_rate, 
       commout_rate, commout_amt, ovout_rate, ovout_amt,  
       commout1_taxamt, commout2_taxamt, commout_taxamt, ovout_taxamt, ovout1_taxamt, ovout2_taxamt
        from static_data."Policies" 
     WHERE id = :polid ;`,
      {
        replacements: {
          polid: policy.previousid
...
VALUES(null, :previousid, :requestNo, :category, 'N',  :usercode, null, null, now() ) returning id;`,
    ...
    let old_value = oldPolicy[0]
    let value ={}
    Object.keys(old_value).forEach(key => {
      value[key] = policy[key]
    })
...
const endorseComov = async (t, policy, usercode) => {
 console.log('------ start endorseComov ---------');

  const currentdate = getCurrentDate()
  try {
    //update policy status ='C'
    policy.edtypecode === "MT83"
...
where bj.request_no = :request_no ;`,
   ...
   //#region insert juiepc juiedt juiepm
    ...
    //  ค่าเบี้ยเท่าเดิมเลยไม่บันทึกลง b_juiepms
    ...
    //   }
    // )
  console.log(">>> done insert juiepc juiedt juiepm ");
    const jupgr = {}
    jupgr.advisor = policy.installment.advisor
...
//MT99
const requestEdtPolicyDetail = async (req, res) => {
  console.log(`----------- begin requestEdtPolicyDetail()  ----------------`);
  const jwt = req.headers.authorization.split(' ')[1];
  const usercode = decode(jwt).USERNAME;
  const appNo = []
  const data = req.body
    ...
    try {
      ...
      -- ON CONFLICT ON CONSTRAINT "idCardNo" DO NOTHING  RETURNING "id" `,
       ...
       console.log(`>>> [SELECT] old_location`);
      const old_location = await sequelize.query(
        `select  "t_location_1"  , "t_location_2" ,"t_location_3" ,"t_location_4" ,"t_location_5" 
        ,(select t_provincename from static_data.provinces where "provinceid" = l."provinceID" limit 1) as province
        ,(select t_amphurname from static_data."Amphurs" where "amphurid" = l."districtID" limit 1) as district
        ,(select t_tambonname from static_data."Tambons" where "tambonid" = l."subDistrictID" limit 1) as subdistrict
        ,zipcode ,"telNum_1"
        ,id as locationid
        from  static_data."Locations" l 
        where "entityID" = :entityID and "locationType" = 'A' and lastversion = 'Y'`,
        {
          replacements: {
            entityID: data.entityID,
          },
      ...
      console.log(`>>> [SELECT] old_policy`);
      const old_policy = await sequelize.query(
        `select "insurerCode", "agentCode", "agentCode2",  "insureID", int."class" , int."subClass"
          , "actDate", "expDate", grossprem, netgrossprem, duty, tax, totalprem
          , commin_rate, commin_amt, ovin_rate, ovin_amt, commin_taxamt, ovin_taxamt 
          , commout_rate, commout_amt, ovout_rate, ovout_amt, commout_taxamt, ovout_taxamt
          , commout1_rate, commout1_amt, ovout1_rate, ovout1_amt, commout1_taxamt, ovout1_taxamt
          , commout2_rate, commout2_amt, ovout2_rate, ovout2_amt, commout2_taxamt, ovout2_taxamt
          , specdiscamt, cover_amt, withheld
          , duedateinsurer as "dueDateInsurer", duedateagent as "dueDateAgent", "fleetCode" 
          , "policyNo", "policyDate", "seqNoins", "seqNoagt", "policyType", "applicationNo"
        from static_data."Policies" pol
        left join static_data."InsureTypes" int on int.id = pol."insureID"
      ...
      console.log(`>>> [SELECT] oldInstallAdvisor`);
      const oldInstallAdvisor = await sequelize.query(
      `select 	bj.netgrossprem as netgrossprem  ,bj.netgrossprem  as grossprem ,bj.specdiscamt 
		,bj.withheld  , bj.duty ,bj.tax 
		,bj.totalprem  , TO_CHAR(bj.totalprem, 'FM99,999,999.00') as totalpremstr
...
VALUES(null, :previousid, :requestNo, :category, 'N',  :usercode, null, null, now() ) returning id;`,
    ...
    const keyidm = b_juiedts[0][0].id
       ...
       let value ={}
       //#region entity
    Object.keys(old_entity[0]).forEach(key => {
      if(old_entity[0][key] !== data[key]){
        value[key] = data[key]
        old_value[key] = old_entity[0][key]
      }
    })
     await sequelize.query(
      `INSERT INTO static_data.b_juiedtds
...
, null, null, null, null, null, null);`,
      {
        replacements: {
          keyidm: keyidm,
          category: 'Entity_Insuree',
          value: JSON.stringify(value),
          old_value: JSON.stringify(old_value),
          table: 'static_data."Entities"',
          keyid: data.entityID
        },
        transaction: t,
        type: QueryTypes.INSERT
    ...
    //#endregion
      ...
      old_value = {}
       value ={}
    Object.keys(old_location[0]).forEach(key => {
      if(old_location[0][key] !== data[key]){
        value[key] = data[key]
        old_value[key] = old_location[0][key]
      }
    })
     await sequelize.query(
      `INSERT INTO static_data.b_juiedtds
...
, :tableM1, :keyidM1, null, null, null, null);`,
      {
        replacements: {
          keyidm: keyidm,
          category: 'Location_Insuree',
          value: JSON.stringify(value),
          old_value: JSON.stringify(old_value),
          table: 'static_data."Locations"',
          keyid: old_location[0].locationid,
          tableM1 : 'static_data."Entities"',
          keyidM1 : data.entityID
...
, "tableM1", "keyidM1", "tableM2", "keyidM2", "tableM3", "keyidM3")
VALUES(:keyidm, :category, null, :value, :old_value, :table, :keyid
, :tableM1, :keyidM1, :tableM2, :keyidM2, null, null);`,
      {
        replacements: {
          keyidm: keyidm,
          category: 'Itemlist_Motors',
          value: JSON.stringify(motorData),
          old_value: JSON.stringify({}),
          table: 'static_data."Motors"',
          keyid: null,
          tableM1 : 'static_data."FleetGroups"',
          keyidM1 : data.itemList,
          tableM2 : 'static_data."Policies"',
          keyidM2 : data.polid
        ...
        type: QueryTypes.INSERT
             ...
             const old_car = await sequelize.query(
        `select id, "brand" as brandname , "voluntaryCode" ,"model" as modelname ,"specname"
        ,"licenseNo" , "chassisNo", "modelYear", "compulsoryCode"
        ,"unregisterflag" , "engineNo", "cc", "seat", "gvw"
        , (select  t_provincename from static_data.provinces  where "motorprovinceID" =  provinceid limit 1) as "motorprovinceID"
        ,email ,"t_ogName"
        ,"taxNo" ,branch ,"t_branchName"
        ...
        Object.keys(old_car[0]).forEach(key => {
      if(old_car[0][key] !== data[key]){
        value[key] = data[key]
        old_value[key] = old_car[0][key]
      }
    })  
     await sequelize.query(
      `INSERT INTO static_data.b_juiedtds
...
, :tableM1, :keyidM1, null, null, null, null);`,
    ...
    //#endregion
      ...
      value ={}
       ...
       Object.keys(old_policy[0]).forEach(key => {
      if(old_policy[0][key] !== data[key]){
        value[key] = data[key]
        old_value[key] = old_policy[0][key]
      }
    })
      await sequelize.query(
      `INSERT INTO static_data.b_juiedtds
...
const endorsePolicyDetail = async (t, policy, usercode,premiumflag = true) => {
 console.log('------ start endorsePolicyDetail ---------');
...
where bj.request_no = :request_no ;`,
    ...
    const update_pol = juiedtds.find(({ table }) => table === `static_data."Policies"`);
    const update_entity_insuree = juiedtds.find(({ table }) => table === `static_data."Entities"`);
    const update_location_insuree = juiedtds.find(({ table }) => table === `static_data."Locations"`);
    const update_jupgr = juiedtds.find(({ table }) => table === `static_data.b_jupgrs`);
    
    let diffnetgrossprem =0;
    let diffduty =0;
    let difftax =0;
    let difftotalprem =0;
    let discinamt =0;
    ...
    for (const key in update_pol.value) {
      policy[key] = update_pol.value[key] // update seqNoagt, endorseNo, specdiscamt
      if(key == "netgrossprem"){diffnetgrossprem = update_pol.old_value[key] - update_pol.value[key]}
      if(key == "duty"){diffduty = update_pol.old_value[key] - update_pol.value[key]}
      if(key == "tax"){difftax = update_pol.old_value[key] - update_pol.value[key]}
      if(key == "totalprem"){difftotalprem = update_pol.old_value[key] - update_pol.value[key]}
      if(key == "specdiscamt"){discinamt = update_pol.old_value[key] - update_pol.value[key]}
    }
    for (const key in update_entity_insuree.value) {
      policy[key] = update_entity_insuree.value[key] // update_entity_insuree
    }
    for (const key in update_location_insuree.value) {
      policy[key] = update_location_insuree.value[key] // update_location_insuree
    }
    policy.installment = update_jupgr.value
 ...
 console.log(">>> done update entity insuree ");
    ...
    await sequelize.query(
      ...
      (:polid, :diffnetgrossprem, :request_no, :diffduty, :difftax, :difftotalprem, :discinamt)`,
      {
        replacements: {
          polid: policy.polid,
          request_no: policy.request_no,
          diffnetgrossprem: diffnetgrossprem,
          diffduty: diffduty,
          difftax: difftax,
          difftotalprem: difftotalprem,
          discinamt: discinamt,
        },
        transaction: t,
        type: QueryTypes.INSERT
  ...
  console.log(">>> done insert juiepc juiedt juiepm ");
    ...
    if (policy.commout2_taxamt == 0) { whtagent2 = 0 }
          ...
          await createjupgrMinor(policy, t, usercode)
          //insert transaction 
  ...
  try {
    ...
    if (req.body.type === 'Approve') {
      ...
      if (req.body.requestFrom !== null && req.body.requestFrom !== '') {
        cond = `${cond} and bj.request_from ='${req.body.requestFrom}' `
      }
    } else if (req.body.type === 'Request') {
      cond = `${cond} and bj.request_from  = '${usercode}'`
    }

    if (req.body.insurerCode !== null && req.body.insurerCode !== '') {
      cond = `${cond} and i."insurerCode"  = '${req.body.insurerCode}'`
    }
    if (req.body.agentCode !== null && req.body.agentCode !== '') {
      cond = `${cond} and a."agentCode" = '${req.body.agentCode}'`
    }

    if (req.body.requestdate_start !== null && req.body.requestdate_start !== '') {
      cond = `${cond} and  bj.request_date between '${req.body.requestdate_start}' and '${req.body.requestdate_end}'`
    }
    ...
    const records = await sequelize.query(
      `select bj.request_no, p."policyNo" ,p."endorseNo" 
	,getname(a."entityID") as "agentName" ,a."agentCode"
	,getname(i."entityID") as "insurerName" ,i."insurerCode"
  ,it."insureName" 
	,bj.request_from , bj.request_date
	--,(case when bj.category = 'Discount' then 'แก้ไขส่วนลด' 
	--	else 'อื่นๆ' end) as category
  , bj.category
  , ( case when bj.status = 'N' then 'รอการอนุมัติ'
  		when bj.status = 'C'  then 'ถูกปฏิเสธ' 
  		when bj.status = 'A'  then 'อนุมัติแล้ว'else 'อื่นๆ' end ) as status 
	 from static_data.b_juiedts bj
join static_data."Policies" p on p.id = bj.previousid
join static_data."InsureTypes" it on it.id =p."insureID" 
join static_data."Insurers" i on i."insurerCode" = p."insurerCode" and i.lastversion = 'Y'
join static_data."Agents" a on a."agentCode" = p."agentCode" and a.lastversion = 'Y'
join static_data."Users" u on u."userName" = bj.request_from 
where ${cond}
    order by bj.request_no ASC `,
      {
...
const getDetailRequestList = async (req, res) => {
  try {
    const jwt = req.headers.authorization.split(' ')[1];
    const usercode = decode(jwt).USERNAME;
 //#region getdata
    const records = await sequelize.query(
      `select bj2.id, bj2.value ,bj2.old_value 
      ,bj.note,bj.category
  --(case when bj2."table" = 'static_data."Policies"' then 'Policy'
   -- when bj2."table" = 'static_data.b_jupgrs' then 'Invoice' 
   -- else 'Other' end) as tag  
    ,bj2."table" as tag  
    from static_data.b_juiedts bj 
join static_data.b_juiedtds bj2 on bj.id = bj2.keyidm 
where bj.request_no = :request_no order by bj.category;`,
      {
        replacements: {
 ...
 //#endregion 
    if (records.length > 0) {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        data.push(ele)
        // recursivediff(ele, data, ele['old_value'], ele['value'])
        // Object.keys(ele['old_value']).forEach(key => {
        //   let seq = 1;
        //   if (ele.tag === 'Invoice') {
        //   //  let maxseq = 0;
        //   console.log(`>>> value.length : ${ele['value'].advisor.length}, old_value.length : ${ele['old_value'].advisor.length}`);
          
        //    let maxele =[]
        //    let minele =[]
        //    if (ele['value'].advisor.length > ele['old_value'].advisor.length ) {  
        //     maxele = ele['value'].advisor
        //     minele = ele['old_value'].advisor
        //   }else{ 
        //     maxele = ele['old_value'].advisor
        //      minele = ele['value'].advisor 
        //   }
        //   console.log(`>>> maxele.length : ${maxele.length}, minele.length : ${minele.length}`);
          
        //   for (let j = 0; j < maxele.length ; j++) {
        //     console.log(`>>> j count : ${j}`);
        //     console.log(`>>> old_value count j: ${j} ,  ${ele['old_value'].advisor[j] != undefined}`);
            
        //     Object.keys(maxele[j]).forEach(key2 => {
        //     if (minele[j]) {
        //        if (maxele[j][key2] != minele[j][key2]) {
        //       data.push({
        //         id: seq,
        //         field: key2,
        //         old_value: ele['old_value'].advisor[j][key2] || "",
        //         new_value: ele['value'].advisor[j][key2] || "",
        //       })
        ...
        //       if(ele['old_value'].advisor[j]  != undefined){oldval = ele['old_value'].advisor[j][key2]}
        //       if(ele['value'].advisor[j] != undefined ){newval = ele['value'].advisor[j][key2]}
        //       data.push({
        //         id: seq,
        //         field: key2,
        //         old_value: oldval,
        ...
        //    }
        //   } else {
        //     if (ele['value'][key] != ele['old_value'][key]) {
        //       data.push({
        //         id: seq,
        //         field: key,
        //         old_value: ele['old_value'][key],
        //         new_value: ele['value'][key],
        //       })
        //       seq++;
        //     }
        //   }
        ...
        // })
        detailList[ele.tag] = data

      }
      resp.detailList = detailList
      resp.note = records[0].note || ''
    } else {
      resp.note = ''
    }
...
where request_no = :request_no ; `, {
    ...
    await t.commit()
    res.json({ message: `reject request_no : ${req.body.request_no} success!!` })
  } catch (error) {
...
const approveRequest = async (req, res) => {
  ...
  try {
    const jwt = req.headers.authorization.split(' ')[1];
    const usercode = decode(jwt).USERNAME;
    console.log('------ start approveRequest ---------');
    ...
    if (req.body.category === 'Discount') {
      const b_juiedts = await sequelize.query(
        ...
        from static_data.b_juiedts iedt
    join static_data."Policies" p on p.id = iedt.previousid  
    ...
    join  static_data."Insurees" i on id = :entityID
     join  static_data."Entities" on id = :entityID
      ...
      // b_juiedts[0].comment = req.data.comment
      await endorseSpecdisc(t, b_juiedts[0], usercode)
    } else if (req.body.category === 'Commov') {
      const b_juiedts = await sequelize.query(
        `select p.*, iedt.previousid ,iedt.request_no , t."policyNo" ,t.dftxno  from static_data.b_juiedts iedt
    join static_data."Policies" p on p.id = iedt.previousid  
    left join static_data."b_jupgrs" bj on bj.polid = p.id
    left join  static_data."Transactions" t on t."policyNo" = bj."policyNo" 
        and t."dftxno" = bj."dftxno" and t."seqNo" = bj."seqNo"  and t.status ='N' 
      ...
      // b_juiedts[0].comment = req.data.comment
      await endorseComov(t, b_juiedts[0], usercode)
    } else if (req.body.category === 'ALL') {
      const b_juiedts = await sequelize.query(
        `select p.*, iedt.previousid ,iedt.request_no , t."policyNo" ,t.dftxno  
        ,"personType" , "titleID" ,"t_firstName" ,"t_lastName"
        ,"idCardType" ,"idCardNo" ,email ,"t_ogName"
        ,"taxNo" ,branch ,"t_branchName",i."entityID"
        ,"t_location_1"  , "t_location_2" ,"t_location_3" ,"t_location_4" ,"t_location_5" 
        ,(select t_provincename from static_data.provinces where "provinceid" = l."provinceID" limit 1) as province
        ,(select t_amphurname from static_data."Amphurs" where "amphurid" = l."districtID" limit 1) as district
        ,(select t_tambonname from static_data."Tambons" where "tambonid" = l."subDistrictID" limit 1) as subdistrict
    ...
    left  join  static_data."Entities" ent on ent.id = i."entityID" 
    left join static_data."Locations" l on l."entityID" =ent.id and l."locationType" ='A' and l.lastversion ='Y'
        where request_no = :request_no
        and bj.installmenttype ='A'
        and t."transType" ='PREM-IN' 
        and t."seqNo" = 1 
      ...
      // b_juiedts[0].comment = req.data.comment
      await endorsePolicyDetail(t, b_juiedts[0], usercode)
    }else {
      throw new Error('ไม่พบ catagory ในระบบ');
    }

    await t.commit()
    await res.json({ message: `Approve request_no : ${req.body.request_no} success!!` })
  } catch (error) {
